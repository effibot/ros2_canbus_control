...
static int frame_recv(int tty_fd, unsigned char *frame, int frame_len_max)
{
    ...
    if ((frame_len == 20) && (frame[0] == 0xaa) && (frame[1] == 0x55)) {
        ...
        if (checksum != frame[frame_len - 1]) {
            fprintf(stderr,
                "frame_recv() failed: Checksum incorrect\n");
            return -1;
        }
    }
...
static int command_settings(int tty_fd, CANUSB_SPEED speed, CANUSB_MODE mode,
                 CANUSB_FRAME frame)
{
    int cmd_frame_len;
...
static int send_data_frame(int tty_fd, CANUSB_FRAME frame, unsigned char id_lsb,
...
#define MAX_FRAME_SIZE 13
    ...
    if (data_length_code < 0 || data_length_code > 8) {
        fprintf(stderr,
            "Data length code (DLC) must be between 0 and 8!\n");
        return -1;
    }
...
static int convert_from_hex(const char *hex_string, unsigned char *bin_string,
                int bin_string_len)
{
    int n1, n2, high;
    ...
    while (hex_string[n1] != '\0') {
        if (hex_value(hex_string[n1]) >= 0) {
            if (high == -1) {
                ...
                bin_string[n2] = hex_value(high) * 16 +
                         hex_value(hex_string[n1]);
                n2++;
...
static int inject_data_frame(int tty_fd, const char *hex_id,
                  const char *hex_data)
{
    int data_len;
    ...
    gap_ts.tv_nsec = (long)(((long long)(inject_sleep_gap * 1000000)) %
                1000000000LL);
    ...
    case 1:
        ...
        break;
    case 2:
        binary_id_lsb = (hex_value(hex_id[0]) * 16) +
                hex_value(hex_id[1]);
        break;
    case 3:
        binary_id_msb = hex_value(hex_id[0]);
        binary_id_lsb = (hex_value(hex_id[1]) * 16) +
                hex_value(hex_id[2]);
        break;
    default:
    ...
    while (program_running && !error) {
        ...
        } else if (inject_payload_mode ==
               CANUSB_INJECT_PAYLOAD_MODE_INCREMENTAL) {
            for (i = 0; i < data_len; i++)
                binary_data[i]++;
        }

        error = send_data_frame(tty_fd, CANUSB_FRAME_STANDARD,
                    binary_id_lsb, binary_id_msb,
