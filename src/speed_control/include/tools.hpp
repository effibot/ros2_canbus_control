...
int sendSDO(int tty_fd, int node_id, SDO_PROTOCOLS prot, SDO_TRANSFER_TYPE transfer_type, int index,
    int subindex, unsigned char* data, int data_len);

int mapPDO(int tty_fd, int node_id, PDO_TYPE PDOtype, int PDOnumber, unsigned char data[],
    int data_len);
