...
namespace USBCANBridge {
    ...
    void VariableFrame::init_fixed_fields() {
        ...
        // * [END]
        storage_.end()[-1] = to_byte(Constants::END_BYTE);
    }
    /**
    ...
    Result<Status> VariableFrame::impl_set_frame_type(const FrameType& frame_type) {
        ...
        // Set the FrameType field in the storage
        storage_[1] = (storage_[1]& std::byte{0x1F}) | to_byte(frame_type);
        return Result<Status>::success(Status::SUCCESS);
    }
    /**
    ...
    Result<Status> VariableFrame::impl_set_frame_fmt(const FrameFmt& frame_fmt) {
        ...
        // Set the FrameFmt field in the storage
        storage_[1] = (storage_[1]& std::byte{0xEF}) | to_byte(frame_fmt);
        return Result<Status>::success(Status::SUCCESS);
    }
    /**
    ...
    Result<Status> VariableFrame::validate_dlc(const std::size_t& dlc) const {
        ...
        // check if the data associated with the DLC fits in the current storage size
        size_t id_size = (impl_get_frame_type().value == FrameType::EXT_VAR) ? 4 : 2;
        size_t required_size = 2 + id_size + dlc + 1; // START + TYPE + ID + DATA + END
