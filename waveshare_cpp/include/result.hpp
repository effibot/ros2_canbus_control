...
template <typename T> struct Result {
  ...
  bool fail() const { return status != Status::SUCCESS; }
  explicit operator bool() const { return ok(); }
  bool operator!() const { return !ok(); }
  std::string to_string() { return status.message(); }
  static Result success(T val) { return {std::move(val), Status::SUCCESS}; }
  static Result error(Status err) { return {T{}, err}; }
};
